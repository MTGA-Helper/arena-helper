...
## Local build without Visual Studio

Use Node.js 22 or newer. The project uses `better-sqlite3` with prebuilt
binaries for supported Node versions, so Visual Studio and `node-gyp` are not
required.
...
Generate JSON and SQLite metadata with `npm start`. To generate JSON only,
skip the optional SQLite output:

```bash
MTGATOOL_SKIP_SQLITE=1 npm start
```

On PowerShell:

```powershell
$env:MTGATOOL_SKIP_SQLITE = "1"
npm start
```
